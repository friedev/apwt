...
public abstract class Menu<WindowType extends Window>
{
    ...
    /**
     * Designates which indices of the {@link Window} can be traversed as a menu. If empty or null, all of the window
     * may be navigated. Otherwise, only indices specified in this list may be selected.
     */
    ...
    {
        this.window = window;
        selection = Math.max(0, Math.min(window.getContents().size() - 1, initialSelection));
        restrictions = new ArrayList<>();
    }

    /**
     * Creates a {@link Menu} from a {@link Window}, starting with the first item selected.
     *
     * @param window the {@link Menu}'s {@link Window}
     */
    ...
    /**
     * Returns the {@link Menu}'s {@link Window}, modified to show which item is selected.
     *
     * @return the {@link Menu}'s {@link Window}, modified to show which item is selected
     */
    ...
    /**
     * Returns true if the given value is a valid index of the {@link Window}'s contents.
     *
     * @param index the value to check for validity as an index
     * @return true if there is an item {@link Window}'s contents at the given index
     */
    ...
    private int bypassRestrictions(int change)
    {
        ...
        do
        {
            ...
            {
                nextSelection = change > 0 ? 0 : getWindow().getContents().size() - 1;
            }

            loop = getWindow().getContents().get(nextSelection) == null || (hasRestrictions() && !restrictions.contains(
                    nextSelection));
